...
public class Playgroup {
	...
	public void addChild(String fname,String lname, Time time,String allergies,
			LocalDate DOB,Parent parent,String num) {
		boolean found=false;
	...
	public void editEmergencyContact(String fname,String lname,LocalDate DOB, String num){
		if(childrenList.isEmpty()) {
			System.out.println("No children in the system.");
			return;
		}
	...
	/*
	public void listAllChildrenForAParent(String fname, String lname, String phoneNum) {
		Parent parent=null;
		boolean found=false;
		if(parentList.isEmpty()) {
			return;
		...
		for(int i=0;i<parentList.size();i++) {
			if(parentList.get(i).isSameParent(fname,lname,phoneNum)) {
		...
		List<Object>temp=search(fname+' '+lname+' '+phoneNum);
		ArrayList<Child> foundChildren;
		for(Object obj:temp) {
			if(obj instanceof Child) {
				foundChildren.add((Child) obj);
			}
		}
